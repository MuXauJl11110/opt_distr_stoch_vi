...
class RobustLinearOracle(BaseSmoothSaddleOracle):
    ...
    def grad_x(self, z: ArrayPair) -> np.ndarray:
        ...
        den = self._n if self.normed else 1.0
        return (
            self.matvec_ATx(w) + self._ones.dot(w) * delta
        ) / den + self.regcoef_x * x
...
def create_robust_linear_oracle(
    A, b: np.ndarray, regcoef_x: float, regcoef_delta: float, normed: bool
):
    matvec_Ax = lambda x: A.dot(x) if isinstance(A, np.ndarray) else A.tocsr() * x
    matvec_ATx = (
        lambda x: A.T.dot(x) if isinstance(A, np.ndarray) else A.tocsr().transpose() * x
    )

    return RobustLinearOracle(
        matvec_Ax, matvec_ATx, b, regcoef_x, regcoef_delta, normed
    )
