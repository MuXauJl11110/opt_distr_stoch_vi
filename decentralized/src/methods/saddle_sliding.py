...
class SaddlePointOracleRegularizer(BaseSmoothSaddleOracle):
    ...
    def func(self, z: ArrayPair) -> float:
        return (
            self.eta * self.oracle.func(z)
            + 0.5 * (z.x - self.v.x).dot(z.x - self.v.x)
...
class DecentralizedSaddleSliding(BaseSaddleMethod):
    ...
    def step(self):
        self.grad_list_z = self.oracle_grad_list(self.z_list)
        grad_av_z = self.acc_gossip(self.grad_list_z, self.con_iters_grad)
        m = np.random.randint(0, self._num_nodes, size=1)[0]
        grad_z_m = ArrayPair(self.grad_list_z.x[m], self.grad_list_z.y[m])
