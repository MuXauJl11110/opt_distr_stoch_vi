...
from runners import DecentralizedExtragradientGTRunner
...
def run_extragrad_gt(
    ...
    if stepsize_factor is not None:
        ...
        print(f"Running decentralized extragradient with stepsize_factor: {stepsize_factor}...")
    else:
        print("Running decentralized extragradient...")

    extragrad_runner.create_method(z_0)
    extragrad_runner.logger.comm_per_iter = 2
    extragrad_runner.run(max_iter=comm_budget_experiment // extragrad_runner.logger.comm_per_iter)
