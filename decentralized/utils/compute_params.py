...
def compute_robust_linear_normed_L(
    A: np.ndarray, b: np.ndarray, r_x: float, r_y: float, regcoef_x: float, regcoef_y: float
) -> float:
    """
...
def compute_robust_linear_normed_delta(A: np.ndarray, Am: np.ndarray, r_x: float, r_y: float, num_parts: int) -> float:
    """
...
def compute_robust_linear_normed_L_delta_mu(
    ...
    for i, size in enumerate(part_sizes):
        ...
        delta_list[i] = compute_robust_linear_normed_delta(A, A[start : start + size], r_x, r_y, n_parts)
        start += size
...
def compute_lam(
    matrix: np.ndarray, eps_real: Optional[float] = 0.00001, eps_imag: Optional[float] = 0.00001
) -> Tuple[float, float]:
    """
